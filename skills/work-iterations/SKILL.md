...
- `work` with `action: list_iterations`: List iterations for the project.
- `work` with `action: list_team_iterations`: List iterations currently assigned to a team.
- `work_iteration_write` with `action: create`: Create iterations in the project.
- `work_iteration_write` with `action: assign`: Assign iterations to a team.
...
- Do **not** call `work_iteration_write` with `action: create` or `action: assign` in this case.
...
- When the user asks to **create iterations** for a project, use `work_iteration_write` with `action: create`.
...
- "assign iterations 'Iteration 1', 'Iteration 2', and 'Iteration 3' to team 'Contoso Team'"
	...
	- Call `work_iteration_write` with `action: assign` and the list of iterations to assign those iterations to the team.

### Example 2

- "assign all iterations in 2026 and 2027 to team 'Contoso Team'"

	- Call `work` with `action: list_iterations` to get the full list of iterations.
	- From the list, find the iterations that fall within 2026 and 2027.
	- Take the iterations that fall within 2026 and 2027 and assign them to the team "Contoso Team" using `work_iteration_write` with `action: assign`.
