...
- `wit_work_item` with `action: my`: Get work items from Azure DevOps that are relevant to the authenticated user.
- `wit_work_item` with `action: get_batch`: Get work item details in batch by IDs. Use this after collecting IDs from `wit_work_item` with `action: my`.
...
in a table format. Show the most recently changed work items first and group the work items by their work item type (System.WorkItemType). For example, group the work items into the following categories based on their work item type:
...
3. Once a project name is obtained, call the `wit_work_item` tool with `action: my` to retrieve work items relevant to the authenticated user for that project.
